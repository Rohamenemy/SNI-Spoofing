...
FAKE_SNIs = config.get("FAKE_SNIs", ["auth.vercel.com"])
BYPASS_METHODS = config.get("BYPASS_METHODS", ["wrong_seq"])
...
async def handle(incoming_sock: socket.socket, incoming_remote_addr):
    ...
    try:
        ...
        current_sni = random.choice(FAKE_SNIs).encode()
        current_method = random.choice(BYPASS_METHODS)
        
        if DATA_MODE == "tls":
            fake_data = ClientHelloMaker.get_client_hello_with(os.urandom(32), os.urandom(32), current_sni,
                                                               os.urandom(32))
        else:
        ...
        if current_method in ["wrong_seq", "ttl_expire"]:
            try:
...
async def main():
    ...
    print(f"\n{Colors.PURPLE}✦ {Colors.LIME}Dar hale goosh dadan rooye {Colors.CYAN}{LISTEN_HOST}:{LISTEN_PORT} {Colors.YELLOW}{Colors.PURPLE}✦{Colors.RESET}\n")
