...
if not os.path.exists(config_path):
    print(f"{Colors.YELLOW}File Config Peyda Nashod Eshkal Nadare Yeki Misazim{Colors.RESET}")
    
    user_ip = input(f"{Colors.CYAN}Connect IP Ro vared Konid (:  {Colors.RESET}").strip()
    connect_ip = user_ip if user_ip else "188.114.98.0"
    
    user_sni = input(f"{Colors.CYAN} Lotfan FakeSNI Ro Vared Koned : {Colors.RESET}").strip()
    ...
    print(f"{Colors.GREEN}[+] File Config.json Add Shod!{Colors.RESET}\n")
...
async def handle(incoming_sock: socket.socket, incoming_remote_addr):
    ...
    try:
        ...
        if DATA_MODE == "tls":
            ...
            sys.exit(f"{Colors.RED}impossible mode!{Colors.RESET}")
        ...
        if BYPASS_METHOD == "wrong_seq":
            try:
                ...
                if fake_injective_conn.t2a_msg == "fake_data_ack_recv":
                    if not has_connected_once:
                        print(f"{Colors.GREEN}[+] Connect: Tunnel to {CONNECT_IP}:{CONNECT_PORT} established.{Colors.RESET}")
                        has_connected_once = True
                else:
                    sys.exit(f"{Colors.RED}impossible t2a msg!{Colors.RESET}")
            except Exception:
                fake_injective_conn.monitor = False
            ...
            sys.exit(f"{Colors.RED}unknown bypass method!{Colors.RESET}")
...
async def main():
    ...
    print(f"{Colors.CYAN}Listening on {LISTEN_HOST}:{LISTEN_PORT}...{Colors.RESET}")
...
if __name__ == "__main__":
    ...
    print(f"@RohamEnemy{Colors.RESET}\n")
    
    try:
        asyncio.run(main())
    except KeyboardInterrupt:
        print(f"\n{Colors.YELLOW}by (:{Colors.RESET}")
        sys.exit(0)
